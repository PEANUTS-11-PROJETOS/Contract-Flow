...
export default function EditarContratoPage() {
  ...
  useEffect(() => {
    supabase.from('contratos').select('*').eq('id', id).single().then(({ data }) => {
      if (data) setForm({
        tipo:              data.tipo ?? data.titulo ?? '',
        ...
        alerta_dias:       data.alerta_dias != null ? String(data.alerta_dias) : '7',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('contratos').update({
      titulo:            form.tipo || form.descricao || 'Contrato',
      tipo:              form.tipo || null,
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--fg)' }}>Editar contrato</h1>
        <p style={{ fontSize: 14, color: 'var(--muted-fg)', marginTop: 4 }}>Altere os dados do contrato</p>
      </div>

      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {/* Tipo */}
        <div style={cardStyle}>
          <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted-fg)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            Tipo de contrato
          </p>
          ...
          {!TIPOS.includes(form.tipo) && form.tipo && (
            <p style={{ fontSize: 12, color: 'var(--muted-fg)' }}>Tipo atual: <strong>{form.tipo}</strong></p>
          )}
        </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <Field label="Valor (R$)" required>
              <input style={inputBase} value={form.valor_total}
                onChange={e => set('valor_total', e.target.value)}
                placeholder="0,00" inputMode="decimal" onFocus={onFocus} onBlur={onBlur} />
            </Field>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            ...
            <Field label="Status">
              <select style={inputBase} value={form.status} onChange={e => set('status', e.target.value)} onFocus={onFocus} onBlur={onBlur}>
        ...
        <div style={cardStyle}>
          ...
          <Field label="Nome" required>
            <input style={inputBase} value={form.cliente_nome}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <Field label="CNPJ / CPF">
              <input style={inputBase} value={form.cliente_documento}
                onChange={e => set('cliente_documento', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
            </Field>
            <Field label="E-mail">
              <input type="email" style={inputBase} value={form.cliente_email}
                onChange={e => set('cliente_email', e.target.value)} onFocus={onFocus} onBlur={onBlur} />
            </Field>
          </div>
        </div>
        ...
        <div style={cardStyle}>
          <Field label="Observações">
            <textarea
              style={{ ...inputBase, height: 'auto', padding: '10px 14px', resize: 'none' }}
